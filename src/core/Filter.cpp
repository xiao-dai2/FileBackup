...
bool FilterRule::match(const fs::path& filePath) const {
    ...
    for (const auto& type : includeTypes) {
        ...
        if (type == "普通文件" && fs::is_regular_file(status)) typeMatch = true;
        if (type == "目录文件" && fs::is_directory(status)) typeMatch = true;
        if (type == "符号连接" && fs::is_symlink(status)) typeMatch = true;
        // 支持管道、设备文件等（扩展）
    }
